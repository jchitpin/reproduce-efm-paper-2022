...
function correlate_efm_values(#
  mc_wt::Vector{Float64},
  mc_ad::Vector{Float64},
  combined_wt,
  combined_ad,
  type::Symbol,
  fname1::String
)
  @assert(length(combined_wt) == 5, "Expected results from 5 objective function.")
  @assert(length(combined_ad) == 5, "Expected results from 5 objective function.")

  # Order data by Markov wildtype and transform and remove infinities
  #idx_mc = sortperm(mc_wt)
  if type ∈ [:efms_raw_log, :efms_prop_log]
    g1(x) = replace(y -> isinf(y) ? -900 : y, x)
  ...
  # Export correlation scatterplot
  CSV.write(#
    fname1,
    Tables.table(#
      hcat(#
        combined_vals_wt, combined_vals_ad, methods, solvers, efms_idx
      )
    ),
    header = ["x_wt", "y_ad", "class", "solver", "efm"],
    delim = '\t'
  )
end
...
function groupsort(M::Matrix{<:Real}, y1::Int64, c1::Int64)
  ...
  # Find indices according to column c1 (smallest to greatest)
  z = sort(M[:,c1])
  idx = sortperm(M[:,c1])
  subset2 = [findlast(x -> x == i, z) for i in unique(z)]
  subset1 = subset2 .+ 1
  subset1 = [1; subset1[1:(end-1)]]
  subset = hcat(subset1, subset2)

  # Sort y1 into groups of c1 (smallest to greatest)
  w = Vector{Int64}()
  x = Vector{Int64}()
  y = Vector{Float64}()
  k = 0
  for i in 1:size(subset,1)
    append!(x, (k+1):(length(subset[i,1]:subset[i,2]) + k))
    idx_sub = sortperm(dat[:,y1][idx[subset[i,1]:subset[i,2]]])
    append!(y, dat[:,y1][idx[subset[i,1]:subset[i,2]]][idx_sub])
    append!(w, idx[subset[i,1]:subset[i,2]][idx_sub])
    k = maximum(x)
  end
  return w, x, y, z
end
